...
// Berechnung der durchschnittlichen Kosten für Hygiene.
$hygiene1  		= $db->query("SELECT SUM(entry_value) AS sum FROM money_entries WHERE entry_category == 5 AND entry_date < '".$date."'");
$hygiene1  		= $hygiene1->fetchAll();
$hygiene   		= $hygiene1[0]["sum"]/($diff_month-2);
...
$gesamt = ($miete + $lebensmittel + $hygiene + $internet + $strom_gas + $gez + $einrichtung);
...
echo "
	...
	<h1>Monatliche Kosten</h1>
		<ul>
			...
			* Hygieneartikel ".round($hygiene,2).	"&euro;<br>
